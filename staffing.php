...
$import = ['auth', 'csrf', 'view', 'html', 'text', 'user', 'notify', 'audit'];
...
$app->view->start('Staffing', 'locker', 'admin');
...
echo '<table class="list roll lt sans"><tr><th>Name</th><th>Type</th><th>Status</th><th></th></tr>';
$cc = 'lr';
foreach ($app->user->listByFacility($app->auth->facilityId()) as $row) {
    echo '<tr class="' . $cc . '"><td>' . h($row['name']) . '</td><td>' . h($row['type']) . '</td><td>' . h($row['status']) . '</td><td>';
    ...
    if ($app->auth->canManageAccount($row)) {
        echo button('Edit', 'Edit account', 'account.php?u=' . (int) $row['id'], 'editNoteButton');
    }
    echo '</td></tr>';
    $cc = $cc === 'lr' ? 'dr' : 'lr';
}
