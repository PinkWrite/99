...
// Original query
// $q = "SELECT id, editor_id, name, code FROM blocks WHERE status='open' ORDER BY id DESC";
...
$sql_cols = 'id';
$sql_where = (isset($e_id)) ? "status='open' AND editor_id='$e_id' ORDER BY $order_by" : "status='open' ORDER BY $order_by" ;
...
// Search form
echo '<br>
<form id="searchformsupblocks" action="'.$clean_where_am_i.'" method="get">';
// All GET arguments
foreach ($_GET as $name => $value) {
...
echo '<td>
		...
		<input type="text" name="r" placeholder="Search" form="searchformsupblocks" id="searchbox"';
		echo (isset($search_query)) ? ' value="'.$search_query.'"' : false; // Here from searching?
		echo '>
		<span data-clear-input onclick="searchClearReset(\'searchbox\', \'searchformsupblocks\');" id="searchclear">&times;</span>
		</div>
		</td><td>
		<label style="cursor:pointer;">
			...
			<input type="submit" form="searchformsupblocks" value="Search" hidden>
		</label>
		</td>';
...
if (mysqli_num_rows($r) == 0) {
	echo '<p class="lt sans">No blocks</p>';
	} else {
	...
	while ($row = mysqli_fetch_array($r)) {
		...
		echo '<tr class="'.$cc.'">
			...
			<td>';
		get_switch("Writs", "View all writs in this block", "writs_sup.php", "b", $block_id, "editNoteButton");
		echo '
			</td>
			<td>';
