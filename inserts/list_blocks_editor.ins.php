...
// Search form
echo '<br>
<form id="searchformeditorblocks" action="'.$clean_where_am_i.'" method="get">';
// All GET arguments
foreach ($_GET as $name => $value) {
...
echo '<td>
		...
		<input type="text" name="r" placeholder="Search" form="searchformeditorblocks" id="searchbox"';
		echo (isset($search_query)) ? ' value="'.$search_query.'"' : false; // Here from searching?
		echo '>
		<span data-clear-input onclick="searchClearReset(\'searchbox\', \'searchformeditorblocks\');" id="searchclear">&times;</span>
		</div>
		</td><td>
		<label style="cursor:pointer;">
			...
			<input type="submit" form="searchformeditorblocks" value="Search" hidden>
		</label>
		</td>';
