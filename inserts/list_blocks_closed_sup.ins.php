...
// Specific editor, such as set from GET on parent page?
if ((isset($e_id)) && (filter_var($e_id, FILTER_VALIDATE_INT, array('min_range' => 1)))) {
	$e_id = preg_replace("/[^0-9]/","", $e_id);
...
// Original query
// $q = "SELECT id, editor_id, name, code FROM blocks WHERE status='closed' ORDER BY id DESC";
...
$sql_cols = 'id';
$sql_where = "status='closed' ORDER BY $order_by";
			...
			<table>
				<tr>
					<td>
						...
						echo "\" title=\"Page 1\" href=\"{$where_am_i}{$sort_get}{$sort_suffix}{$editor_suffix}{$search_suffix}&p=1\">&laquo;</a>
					</td>
					<td>
						<a class=\"paginate";
						if ($paged == 1) {echo " disabled";}
					 echo "\" title=\"Previous\" href=\"{$where_am_i}{$sort_get}{$sort_suffix}{$editor_suffix}{$search_suffix}&p=$prevpaged\">&lsaquo;&nbsp;</a>
					</td>
					<td>
						<a class=\"paginate current\" title=\"Next\" href=\"{$where_am_i}{$sort_get}{$sort_suffix}{$editor_suffix}{$search_suffix}&p=$paged\">Page $paged ($totalpages)</a>
					</td>
					<td>
						<a class=\"paginate";
						if ($paged == $totalpages) {echo " disabled";}
					 echo "\" title=\"Next\" href=\"{$where_am_i}{$sort_get}{$sort_suffix}{$editor_suffix}{$search_suffix}&p=$nextpaged\">&nbsp;&rsaquo;</a>
					</td>
					 <td>
						 <a class=\"paginate";
						 if ($paged == $totalpages) {echo " disabled";}
						echo "\" title=\"Last Page\" href=\"{$where_am_i}{$sort_get}{$sort_suffix}{$editor_suffix}{$search_suffix}&p=$totalpages\">&raquo;</a>
					 </td>
				</tr>
			</table>
		</div>
...
// Search form
echo '<br>
<form id="searchformsupblocksclosed" action="'.$clean_where_am_i.'" method="get">';
// All GET arguments
foreach ($_GET as $name => $value) {
...
set_button("Creation", "Sort by order of creation", "{$where_am_i}{$sort_get}s=creation{$editor_suffix}{$search_suffix}", $creation_cl);
echo '</td><td>';
set_button("Name", "Sort by name", "{$where_am_i}{$sort_get}s=name{$editor_suffix}{$search_suffix}", $name_cl);
echo '</td><td>';
set_button("Code", "Sort by code", "{$where_am_i}{$sort_get}s=code{$editor_suffix}{$search_suffix}", $code_cl);
echo '</td>';
// Search form inputs
echo '<td>
		<div class="search-input">
		<input type="text" name="r" placeholder="Search" form="searchformsupblocksclosed" id="searchbox"';
		echo (isset($search_query)) ? ' value="'.$search_query.'"' : false; // Here from searching?
		echo '>
		<span data-clear-input onclick="searchClearReset(\'searchbox\', \'searchformsupblocksclosed\');" id="searchclear">&times;</span>
		</div>
		</td><td>
		<label style="cursor:pointer;">
			...
			<input type="submit" form="searchformsupblocksclosed" value="Search" hidden>
		</label>
		</td>';
...
$q = "SELECT $sql_cols FROM blocks WHERE $SQLcolumnSearch $sql_where LIMIT $itemskip,$pageitems";
$r = mysqli_query ($dbc, $q);

// Empty?
if (mysqli_num_rows($r) == 0) {
	echo '<p class="lt sans">No closed blocks</p>';
	} else {

	// Start the table
	echo '
	<table class="list sans lt">';
	...
	while ($row = mysqli_fetch_array($r)) {
		...
		// See if any writs are current
		$qw = "SELECT id FROM writs WHERE block=$block_id AND (term_status='current' OR review_status='$review_status')";
		$rw = mysqli_query ($dbc, $qw);

		echo '<tr class="'.$cc.'">
			<td><a class="listed_note" href="block_sup.php?v='.$block_id.'"><b>'.$name.'</b></a></td>
			<td><a class="listed_note" href="block_sup.php?v='.$block_id.'">'.$code.'</a></td>
			<td>'.$editor_name.' <small>('.$editor_username.')</small></td>
			<td>';
		if (mysqli_num_rows($rw) > 0) {
			echo '<form action="archive.act.php" method="post">
			<input type="checkbox" name="checksubmit" value="'.$block_id.'" id="checksubmit'.$block_id.'">
			<b><input type="submit" class="act_blue small" name="block_archive" value="archive all writs" style="display: inline; float: right;"></b>
			</form>';
		}
		echo '</td>
			<td><div style="display: inline; float:right;">';
		set_switch("Edit", "Edit this block", "block_sup.php?v=$block_id", "opened_by", $userid, "editNoteButton");
		echo '</div>
			</td>
			</tr>';
