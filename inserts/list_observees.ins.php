...
// Search form
echo '<br>
<form id="searchformobservees" action="'.$clean_where_am_i.'" method="get">';
// All GET arguments
foreach ($_GET as $name => $value) {
...
echo '<td>
		...
		<input type="text" name="r" placeholder="Search" form="searchformobservees" id="searchbox"';
		echo (isset($search_query)) ? ' value="'.$search_query.'"' : false; // Here from searching?
		echo '>
		<span data-clear-input onclick="searchClearReset(\'searchbox\', \'searchformobservees\');" id="searchclear">&times;</span>
		</div>
		</td><td>
		<label style="cursor:pointer;">
			...
			<input type="submit" form="searchformobservees" value="Search" hidden>
		</label>
		</td>';
