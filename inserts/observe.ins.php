...
if (isset($_GET['w'])) {
	...
	if ($block_id == 0) {
		...
		$list_block = '<h4 class="review">Block: Main <small>('.$editor_name.')</small></h4>';
		...
		$list_block = '<h4 class="review">Block: '.$block_name.' <small>('.$block_code.')</small> with '.$editor_name.'</h4>';
	}
	...
	'.$list_block.'
	<h4 class="review">Score:</h4>
	<section class="writcontent score" id="edits">'.$score.'<small class="dk">/'.$outof.'</small></section>
	<h4 class="review">Scoring remarks:</h4>
	<section class="writcontent remarks" id="edits">'.nl2br(preg_replace("/[\r\n]{2,}/", "\n", $scoring)).'</section>
	<h4 class="review">Draft: <small><i class="dk sans">'.$draft_status.' '.$draft_submit_date.'</i></small></h4>
	...
	<hr class="review" />
	<h4 class="review">Writer correction: <small><i class="dk sans">'.$edits_status.' '.$draft_submit_date.'</i></small></h4>
	...
	<h4 class="review">Scoring remarks: <small><i>(same as above, for reference)</i></small></h4>
