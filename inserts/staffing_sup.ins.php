...
&& (isset($_POST['where_am_i'])) && ($_POST['where_am_i'] == $where_am_i) ) {
	...
	$confirm__input_items = '';
	$confirm__display_items = '';
...
// Who we are managing?
$editors_cl = 'act_ltgray';
$supervisors_cl = 'act_ltgray';
$admins_cl = 'act_ltgray';
if ((isset($_GET['t'])) && (preg_match("/[a-z]/", $_GET['t']))) {
	$filter_type = preg_replace("/[^a-z]/","", $_GET['t']);
	switch ($filter_type) {
		case "editors":
				$head_type = 'Editors';
				$editors_cl = 'act_dkgray';
				$type_suffix = '&t=editors';
				break;
		case "supervisors":
				$head_type = 'Supervisors';
				$supervisors_cl = 'act_dkgray';
				$type_suffix = '&t=supervisors';
				break;
		case "admins":
				$head_type = 'Admins';
				$admins_cl = 'act_dkgray';
				$type_suffix = '&t=admins';
				break;
		default:
				$head_type = 'Editors';
				$editors_cl = 'act_dkgray';
				$type_suffix = '&t=creation';
				break;
	}
} else {
	$filter_type = 'editors';
...
$sql_cols = 'id';
if (($usr_type == "Supervisor") || (($filter_type != 'supervisors') && ($filter_type != 'admins'))) {
	$sql_where = "type='editor' OR (type='supervisor' AND id='$userid')";
} elseif (($usr_type == "Admin") && ($filter_type == 'supervisors')) {
	$sql_where = "type='supervisor'";
} elseif (($usr_type == "Admin") && ($filter_type == 'admins')) {
	$sql_where = "type='admin'";
}
$qp = "SELECT $sql_cols FROM users WHERE $SQLcolumnSearch $sql_where ORDER BY $order_by";
$rp = mysqli_query($dbc, $qp);
$totalrows = mysqli_num_rows($rp);
//DEV put here, this looks ugly and confusing for UX
			...
			<table>
				<tr>
					<td>
						...
						echo "\" title=\"Page 1\" href=\"${where_am_i}${sort_get}${sort_suffix}${type_suffix}${search_suffix}&p=1\">&laquo;</a>
					</td>
					<td>
						<a class=\"paginate";
						if ($paged == 1) {echo " disabled";}
					 echo "\" title=\"Previous\" href=\"${where_am_i}${sort_get}${sort_suffix}${type_suffix}${search_suffix}&p=$prevpaged\">&lsaquo;&nbsp;</a>
					</td>
					<td>
						<a class=\"paginate current\" title=\"Next\" href=\"${where_am_i}${sort_get}${sort_suffix}${type_suffix}${search_suffix}&p=$paged\">Page $paged ($totalpages)</a>
					</td>
					<td>
						<a class=\"paginate";
						if ($paged == $totalpages) {echo " disabled";}
					 echo "\" title=\"Next\" href=\"${where_am_i}${sort_get}${sort_suffix}${type_suffix}${search_suffix}&p=$nextpaged\">&nbsp;&rsaquo;</a>
					</td>
					 <td>
						 <a class=\"paginate";
						 if ($paged == $totalpages) {echo " disabled";}
						echo "\" title=\"Last Page\" href=\"${where_am_i}${sort_get}${sort_suffix}${type_suffix}${search_suffix}&p=$totalpages\">&raquo;</a>
					 </td>
				</tr>
			</table>
		</div>
...
if ($usr_type == "Admin") {
	echo '<td>';
set_button("Editors", "List editors", "${where_am_i}${sort_get}${sort_suffix}&t=editors${search_suffix}", $editors_cl);
echo '</td><td>';
set_button("Supervisors", "List supervisors", "${where_am_i}${sort_get}${sort_suffix}&t=supervisors${search_suffix}", $supervisors_cl);
echo '</td><td>';
set_button("Admins", "List admins", "${where_am_i}${sort_get}${sort_suffix}&t=admins${search_suffix}", $admins_cl);
echo '</td>';
}
...
set_button("Creation", "Sort by order of creation", "${where_am_i}${sort_get}s=creation${type_suffix}${search_suffix}", $creation_cl);
echo '</td><td>';
set_button("Name", "Sort by name", "${where_am_i}${sort_get}s=name${type_suffix}${search_suffix}", $name_cl);
echo '</td>';
// Search form inputs
echo '<td>
...
if (mysqli_num_rows($r) == 0) {
	echo '<p class="lt sans">No '.$filter_type.'</p>';
} else {
	...
	while ($row = mysqli_fetch_array($r)) {
		...
		echo ($u_id == $userid) ? '<tr class="hr">' : '<tr class="'.$cc.'">' ;
		echo '<td><b>'.$u_name.'</b> <small>('.$u_username.') </small>';

		echo ($u_id == $userid) ? ' - <i>(YOU!)</i>' : false ;
		...
		echo '</div>
			...
			<td><div style="display: inline; float:left;">';
		get_switch("Writs", "View all writs under this editor", "writs_sup.php", "w", $u_id, "editNoteButton");
		echo '</div>
			</td>
			<td><div style="display: inline; float:left;">';
		...
		echo '</div>
			...
			<td><div style="display: inline; float:left;">';
		set_switch("Observees", "Edit this user's observees", "change_user_observees.php?v=$u_id", "opened_by", $userid, "editNoteButton");
		echo '</div>
			</td>
			<td><div style="display: inline; float:left;">';
